...
int find_vertex(Vertex *Graph, int i, int j, bool **Matrix,int dont_del_i)
{
    ...
    // cout<<"первая проверка поиска точек "<<del_i<<" "<<endl;
    i=Graph[i].previousVertex;
    while(i!=j)
    {
    ...
    //cout<<endl<<"вторая проверка поиска точек "<<del_i<<" "<<endl;
    return del_i;
...
int main()
{
  int test=0;
    ...
    while(test<5)
   {
    ...
    delete []Matrix;
    delete []Graph;
    for(int i=0;i<p_o_c.size();i++) p_o_c.erase(p_o_c.begin());
    test++;
   }
    return 0;
}
